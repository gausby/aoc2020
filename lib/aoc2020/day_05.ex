defmodule Aoc2020.Day05 do
  ...
  - `BBFFBBFRLL`: row 102, column 4, seat ID 820.
    ...
    iex> Aoc2020.Day05.BoardingPass.parse("BFFFBBFRRR")
    %Aoc2020.Day05.BoardingPass{row: 70, column: 7, id: 567}
    iex> Aoc2020.Day05.BoardingPass.parse("FFFBBBFRRR")
    %Aoc2020.Day05.BoardingPass{row: 14, column: 7, id: 119}
    iex> Aoc2020.Day05.BoardingPass.parse("BBFFBBFRLL")
    %Aoc2020.Day05.BoardingPass{row: 102, column: 4, id: 820}
  ...
  defmodule BoardingPass do
    defstruct [:row, :column, :id]

    def compare(%BoardingPass{id: a}, %BoardingPass{id: b}) do
      cond do
        a < b -> :lt
        a > b -> :gt
        true -> :eq
    ...
    def parse(binary_space_partitioning) do
      row = parse_row(binary_space_partitioning)
      column = parse_column(binary_space_partitioning)
      id = row * 8 + column

      struct(__MODULE__, row: row, column: column, id: id)
    end

    def parse_row(<<row::binary-size(7), _column::binary-size(3), _::binary>>) do
      for <<n::binary-size(1) <- row>>, reduce: 0 do
        acc when n == "F" ->
          acc * 2
    ...
    def parse_column(<<_row::binary-size(7), column::binary-size(3), _::binary>>) do
      for <<n::binary-size(1) <- column>>, reduce: 0 do
        acc when n == "L" ->
          acc * 2
