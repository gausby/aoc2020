defmodule Aoc2020.Day01 do
  ...
  vacation at a nice resort on a tropical island. Surely, Christmas
  ...
  the correct answer is `514579`.
    ...
    iex> reports = [1721, 979, 366, 299, 675, 1456]
    iex> Aoc2020.Day01.find_matches(reports, entries: 2, sum: 2020)
    [514579]
  ...
  answer, `241861950`.
    ...
    iex> reports = [1721, 979, 366, 299, 675, 1456]
    iex> Aoc2020.Day01.find_matches(reports, entries: 3, sum: 2020)
    [241861950]
  ...
  def find_matches(reports, opts \\ []) do
    entries =
      Keyword.get(opts, :entries) ||
        raise ArgumentError, "Please specify the number of entries that should sum to `sum`"

    sum =
      Keyword.get(opts, :sum) ||
        raise ArgumentError, "Please specify the sum of entries that should sum to"

    find_matches_(reports, entries, sum)
  end

  defp find_matches_(reports, 2, sum) do
    for entry <- reports, a1 <- reports -- [entry], a1 + entry == sum, uniq: true do
      entry * a1
    end
  end

  defp find_matches_(reports, 3, sum) do
    for entry <- reports,
        a1 <- reports -- [entry],
        a2 <- reports -- [entry, a1],
        a1 + a2 + entry == sum,
